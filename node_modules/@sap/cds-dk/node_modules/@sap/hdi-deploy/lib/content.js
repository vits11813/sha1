...
function deployFileContent (file, services, stripCRFromCSV) {
  ...
  // To avoid opening too many files at once we use a callback to create the ReadStream
...
class Content {
  ...
  constructor (root, services, workingSet, deployDirs, pathFilter, stripCRFromCSV, excludeFilter) {
    ...
    try {
       ...
       * and the second element is either a function that returns a ReadStream or a Buffer containing the file's content.
       */
      this._deployContent = [].concat(
        dirs.map((dir) => [deployDirPath(dir), dummyBuffer]),
        files
          .filter(isDeployableFile)
          .map((file) => [deployFilePath(file), deployFileContent(file, services, stripCRFromCSV)])
      );
    } catch (e) {
